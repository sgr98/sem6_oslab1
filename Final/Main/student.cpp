...
{
	string file = "./Database/Instructors/" + coursename + "/hist0.txt";
...
void handleButtonClick( string selection )
{
	if( selection == "Download" )
	{
        ...
        f.open(fileName);
        Student student;
        vector<string> course_list = student.getCourses("./Database/Students/" + studentName + ".txt");
        vector< int > marks;
        int i;
        int space = 15;
...
int get_marks(string coursename,string student)
{
	string files = "./Database/Instructors/" + coursename + "/hist0.txt";
    InstructorMarksList instructor_marks_list;
